...
@Composable
internal fun ReaderFilterScreen(
    onBack: () -> Unit,
    navigateToTagPicker: () -> Unit,
    args: ReaderFilterArgs,
    viewModel: ReaderFilterViewModel = hiltViewModel(),
) {
    viewModel.init(args)

    val viewState by viewModel.viewStates.observeAsState(ReaderFilterViewState())
    val viewEffect by viewModel.viewEffects.observeAsState()
    AppThemeM3(darkTheme = viewState.isDark) {
        LaunchedEffect(key1 = viewEffect) {
            when (viewEffect?.consume()) {
                null -> Unit
                is ReaderFilterViewEffect.OnBack -> {
                    onBack()
                }

                is ReaderFilterViewEffect.NavigateToTagPickerScreen -> {
                    navigateToTagPicker()
                }
            }
        }
...
@Composable
private fun ReaderFilterTagsListAndSelect(
    viewState: ReaderFilterViewState,
    viewModel: ReaderFilterViewModel,
) {
...
@Composable
private fun ReaderFilterScreenTopBar(
    onBack: () -> Unit,
    onClear: (() -> Unit)?,
) {
    TopAppBar(
