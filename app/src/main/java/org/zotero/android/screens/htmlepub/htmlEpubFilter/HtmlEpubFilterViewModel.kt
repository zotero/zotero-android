...
@HiltViewModel
internal class HtmlEpubFilterViewModel @Inject constructor(
    private val pdfReaderCurrentThemeEventStream: PdfReaderCurrentThemeEventStream,
) : BaseViewModel2<HtmlEpubFilterViewState, HtmlEpubFilterViewEffect>(HtmlEpubFilterViewState()) {

    private var pdfReaderThemeCancellable: Job? = null

    @Subscribe(threadMode = ThreadMode.MAIN)
    fun onEvent(tagPickerResult: TagPickerResult) {
        if (tagPickerResult.callPoint == TagPickerResult.CallPoint.HtmlEpubFilter) {
    ...
    fun onClose() {
        triggerEffect(HtmlEpubFilterViewEffect.OnBack)
        updateFilter()
    }

    private fun updateFilter() {
        if (viewState.colors.isEmpty() && viewState.tags.isEmpty()) {
            EventBus.getDefault().post(HtmlEpubFilterResult(null))
        } else {
            val filter = HtmlEpubAnnotationsFilter(colors = viewState.colors, tags = viewState.tags)
            EventBus.getDefault().post(HtmlEpubFilterResult(filter))
        }
    }
    ...
    fun onTagsClicked() {
        ScreenArguments.tagPickerArgs = TagPickerArgs(
            ...
            callPoint = TagPickerResult.CallPoint.HtmlEpubFilter,
        )

        triggerEffect(HtmlEpubFilterViewEffect.NavigateToTagPickerScreen)
    }
...
internal sealed class HtmlEpubFilterViewEffect : ViewEffect {
    object OnBack: HtmlEpubFilterViewEffect()
    object NavigateToTagPickerScreen: HtmlEpubFilterViewEffect()
}