package org.zotero.android.loaders.pdfworker
...
class PdfWorkerLoader @Inject constructor(
    ...
    private fun _update() {
        ...
        Timber.Forest.i("PdfWorkerLoader: update pdf-worker JS")
        try {
    ...
    private fun _updatePdfWorkerFromBundle(forceUpdate: Boolean) {
        ...
        Timber.Forest.i("PdfWorkerLoader: should update pdf-worker from bundle, forceUpdate=$forceUpdate; oldHash=${defaults.getLastPdfWorkerCommitHash()}; newHash=$hash")
        if (!forceUpdate && defaults.getLastPdfWorkerCommitHash() == hash) {
            return
        }
        Timber.Forest.i("PdfWorkerLoader: update pdf-worker from bundle")
        updatePdfWorker()
    ...
    private fun updateFromBundle() {
        try {
            ...
            Timber.Forest.e(error, "PdfWorkerLoader: can't update from bundle")
        ...
        try {
            if (!fileStore.pdfWorkerDirectory().exists()) {
                if (type != UpdateType.initial) {
                    Timber.Forest.e("PdfWorkerLoader: pdf-worker directory was missing!")
                }
            ...
            Timber.Forest.e(error, "PdfWorkerLoader: unable to restore folder integrity")
            throw error
        }
    ...
    private fun process(error: Exception) {
        Timber.Forest.e(error, "PdfWorkerLoader: error")
