package org.zotero.android.loaders.citation
...
class CitationProcLoader @Inject constructor(
    ...
    private fun _update() {
        ...
        Timber.Forest.i("CitationProcLoader: update citation_proc JS")
        try {
    ...
    private fun _updateFromBundle(forceUpdate: Boolean) {
        ...
        Timber.Forest.i("CitationProcLoader: should update citation_proc from bundle, forceUpdate=$forceUpdate; oldHash=${defaults.getLastCitationProcCommitHash()}; newHash=$hash")
        if (!forceUpdate && defaults.getLastCitationProcCommitHash() == hash) {
            return
        }
        Timber.Forest.i("CitationProcLoader: update citation_proc from bundle")
        updateCitationProc()
    ...
    private fun updateFromBundle() {
        try {
            ...
            Timber.Forest.e(error, "CitationProcLoader: can't update from bundle")
        ...
        try {
            if (!fileStore.citationDirectory().exists()) {
                if (type != UpdateType.initial) {
                    Timber.Forest.e("CitationProcLoader: citation directory was missing!")
                }
            ...
            Timber.Forest.e(error, "CitationProcLoader: unable to restore folder integrity")
            throw error
        }
    ...
    private fun process(error: Exception) {
        Timber.Forest.e(error, "CitationProcLoader: error")
