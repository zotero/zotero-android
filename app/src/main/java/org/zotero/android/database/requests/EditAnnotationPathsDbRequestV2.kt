...
class EditAnnotationPathsDbRequestV2(
    ...
    override fun process(database: Realm) {
        ...
        if (!pathsChanged(this.paths, item.paths)) {
            return
        }
        sync(paths = this.paths, item = item, database = database)
    }
    ...
    private fun pathsChanged(paths: List<List<PointF>>, itemPaths: List<RPath>): Boolean {
