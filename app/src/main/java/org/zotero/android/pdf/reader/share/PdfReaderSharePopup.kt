        ...
        Column(
            ...
            if (viewState.parentKey != null) {
                PdfReaderSharePopupOptionRow(
                    text = safeStringResource(id = Strings.citation_copy_citation),
                    onOptionClick = viewModel::onCopyCitation,
                    resIcon = Drawables.file_copy
                )
                PdfReaderSharePopupOptionRow(
                    text = safeStringResource(id = Strings.citation_copy_bibliography),
                    onOptionClick = viewModel::onCopyBibliography,
                    resIcon = Drawables.file_copy
                )
            }
            PdfReaderSharePopupOptionRow(
                text = safeStringResource(id = Strings.pdf_export_export),
