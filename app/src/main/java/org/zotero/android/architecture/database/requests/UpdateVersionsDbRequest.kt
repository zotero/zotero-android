        ...
        database.executeTransaction {
            when (this.libraryId) {
                is LibraryIdentifier.custom -> {
                    ...
                    if (versions != null) {
                        update(versions =  versions, type = this.type, version = this.version)
                    }
                }
                is LibraryIdentifier.group -> {
                    val library = database.where<RGroup>().equalTo("identifier", libraryId.groupId).findFirst() ?: throw DbError.objectNotFound
