...
class CreateAttachmentsDbRequest(
    ...
    override fun process(
        ...
        val libraryId  =this.attachments.firstOrNull()?.libraryId
        if (libraryId == null) {
            return emptyList()
        }

        val parent = this.parentKey?.let { database.where<RItem>().key(it, libraryId).findFirst()}
        if (parent != null) {
            parent.version = parent.version
        }

        var failed: MutableList<Pair<String, String>> = mutableListOf()

        for (attachment in this.attachments) {
            try {
                val attachment = CreateAttachmentDbRequest(attachment = attachment, parentKey = null, localizedType = this.localizedType, collections = this.collections, tags = emptyList(), fileStore = fileStore).process(database)
                    if (parent != null) {
