...
class MarkAllLibraryObjectChangesAsSyncedDbRequest constructor(
    ...
    override fun process(database: Realm) {
        deleteObjects(database.where<RItem>().deleted(true, this.libraryId), database)
        deleteObjects(database.where<RCollection>().deleted(true, this.libraryId), database)
        deleteObjects(database.where<RSearch>().deleted(true, this.libraryId), database)

        for (objectS in database.where<RCollection>().changesWithoutDeletions(this.libraryId).findAll()) {
            objectS.deleteAllChanges(database = database)
        }
        for (objectS in database.where<RItem>().changesWithoutDeletions(this.libraryId).findAll()) {
            objectS.deleteAllChanges(database = database)
        }
        for (objectS in database.where<RSearch>().changesWithoutDeletions(this.libraryId).findAll()) {
            objectS.deleteAllChanges(database = database)
        }
