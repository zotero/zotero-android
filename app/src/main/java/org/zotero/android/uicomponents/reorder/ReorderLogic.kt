...
import org.zotero.android.uicomponents.reorder.DragDirection.DOWN
import org.zotero.android.uicomponents.reorder.DragDirection.UP
...
internal class ReorderLogic(
    ...
    private val canDragOver: ((index: Int, dragDirection: DragDirection) -> Boolean)? = null,
    private val onDragStart: ((index: Int) -> (Unit))? = null,
            ...
            state.draggedIndex?.also { draggedItem ->
                chooseDropItem(
                    state.listState.layoutInfo.visibleItemsInfo
                        ...
                        .filter {
                            val isDraggingDown = state.draggedIndex?.let { draggedIndex ->
                                draggedIndex < it.index
                            } == true
                            val dragDirection = if (isDraggingDown) DOWN else UP
                            canDragOver?.invoke(it.index, dragDirection) != false
                        },
                    start,
                    end
                )?.also { targetIdx ->
                    onMove(draggedItem, targetIdx.index)
