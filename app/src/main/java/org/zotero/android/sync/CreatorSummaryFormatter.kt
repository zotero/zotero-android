        ...
        private fun summaryStr(creators: List<RCreator>): String? {
            ...
            when (creators.size) {
                ...
                2 -> {
                    ...
                    return context.getString(Strings.items_creator_summary_and, sorted.first().summaryName, sorted.last().summaryName)
                }

                else -> {
                    val sorted = creators.sortedBy { it.orderId}
                    return context.getString(Strings.items_creator_summary_etal, sorted.first().summaryName)
                }
            }
        }
    }
