...
class RevertLibraryUpdatesSyncAction @AssistedInject constructor(
    ...
    suspend fun result(): CustomResult<Map<SyncObject, List<String>>> {
        try {
            ...
            dbWrapperMain.realmDbStorage.perform { coordinator ->
                ...
                val storeItemsRequest = storeItemsDbResponseRequestFactory.create(
    ...
    fun <T : RealmObject, K> loadCachedJsonForObject(
        ...
        for (objectS in objects) {
            objectS as Syncable
            try {
                val file = fileStore.jsonCacheFile(
                    objectS = objectType,
                    libraryId = libraryId,
                    key = objectS.key
                )
    ...
    interface Factory {
        fun create(
            @Assisted libraryId: LibraryIdentifier
