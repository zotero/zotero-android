...
localesToZipSubFolder = os.path.join(os.path.abspath("."), localesToZipSubFolderName)
...
if not os.path.isdir(localesToZipSubFolder):
    os.mkdir(localesToZipSubFolder)
...
for filename in os.listdir(submodule_dir):
    if filename.endswith(".xml") or filename.endswith(".json"):
        shutil.copyfile(os.path.join(submodule_dir, filename), os.path.join(localesToZipSubFolder, filename))
        continue
    else:
        continue
